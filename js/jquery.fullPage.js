...
(function($, window, document, Math, undefined) {
    ...
    $.fn.fullpage = function(options) {
       ...
       const preloader = document.querySelector('#preloader');
        ...
        const icon = scrollTop.querySelector("i"); // Select the icon once for reuse
        let clicked = false; // Flag to check if the icon has been clicked
        ...
        scrollTop.addEventListener('click', (e) => {
            ...
            // Scale the icon up when clicked
            icon.style.transform = "scale(1.5)"; // Scale up on click
            ...
            // Reset the scale after scrolling is done
            setTimeout(() => {
                if (clicked) {
                    icon.style.transform = "scale(1)"; // Scale back to original size after click
                    clicked = false; // Reset the clicked flag
                }
            }, 500); // Adjust this duration to match your scrolling speed or animation duration
        });

        // Add hover effects
        scrollTop.addEventListener('mouseover', () => {
            if (!clicked) { // Only scale up on hover if not clicked
                icon.style.transform = "scale(1.5)"; // Scale up on hover
            }
        });

        scrollTop.addEventListener('mouseout', () => {
            if (!clicked) { // Only scale down on mouse out if not clicked
                icon.style.transform = "scale(1)"; // Scale back to original size
            }
        });
